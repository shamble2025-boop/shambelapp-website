...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  const title = rawSlug.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
...
export default async function CategoryPage({ params }: { params: { slug: string } }) {
  ...
  const articles = allArticles.filter(a => 
    a.category_en.toLowerCase().replace(/\s/g, '-') === rawSlug ||
    a.category_en.toLowerCase().replace(/\s/g, '-') === params.slug
  );
